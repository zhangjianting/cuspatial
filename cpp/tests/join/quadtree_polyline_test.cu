...
struct QuadtreepolylineBBoxJoinTest : public GdfTest 
{
 
};

TEST_F(QuadtreepolylineBBoxJoinTest, test_small)
{
   ...
   //no need to delete db_pnt_x and db_pnt_y, as they are taken over by mutable_column_views x/y
    ...
    assert(num_vertex=ply_spos[num_poly-1]);
    ...
    CUDF_EXPECTS(num_pq_cols==2,"a polyline-quadrant pair table must have 2 columns (polyline-idx,quadrant-idx)");
