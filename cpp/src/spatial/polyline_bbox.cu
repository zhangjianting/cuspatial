...
struct bounding_box_processor {
        ...
        cudaStream_t stream)
    {
        ...
        rmm::device_buffer *db_temp_spos=new rmm::device_buffer(num_poly* sizeof(uint32_t),stream,mr);
        CUDF_EXPECTS(db_temp_spos!=nullptr, "error allocating temporal memory for first ring position array"); 
...
if(0)
{
        ...
        thrust::transform(exec_policy->on(stream),d_p_bbox,d_p_bbox+num_bbox,out_bbox_iter,bbox2tuple<T>(R));
    ...
    double R)
{
    ...
    return cudf::experimental::type_dispatcher(x.type(),bounding_box_processor{},spos,x,y,R,mr,stream);
}
